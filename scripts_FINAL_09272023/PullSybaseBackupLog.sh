...
#       the messages from the referenced Sybase Backup Error Log on
...
## 
##################################################################################
echo "===========Welcome to PullSybaseBackupLogNEW.sh======================================"
...
##BASEDIR=/home/logicmonc/scripts
...
IsPullRunning=$(ps -aux | grep "ssh logicmonc@$RemoteSybaseHost" | grep $OriginalSybaseBackupLogName | grep -v grep | wc -l)
echo "IsPullRunning: $IsPullRunning"
if [ $IsPullRunning -gt $HowManyActivePulls ]
then
        echo "Sybase Backup Error Log Transfer is active"
else
	echo "================YES we need to START UP the SYB BKP log message pulls!"
        . $BASEDIR/LMSybLogRefresh.sh $EnvFileToUse BKP
fi
echo "===========Goodbye FROM PullSybaseBackupLogNEW.sh======================================"
