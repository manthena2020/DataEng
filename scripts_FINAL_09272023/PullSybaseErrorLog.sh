...
#       the messages from the referenced Sybase Error Log on
...
## 
##################################################################################
echo "===========Welcome to PullSybaseErrorLogNEW.sh======================================"
...
IsPullRunning=$(ps -aux | grep "ssh logicmonc@$RemoteSybaseHost" | grep $OriginalSybaseErrorLogName | grep -v grep | wc -l)
echo "IsPullRunning: $IsPullRunning"
if [ $IsPullRunning -gt $HowManyActivePulls ]
then
        echo "Sybase Error Log Transfer is active so there is nothing to do but exit"
else
	echo "================YES we need to START UP the SYB log message pulls!"
#        . $BASEDIR/LMSybLogRefresh.sh $EnvFileToUse ASE
         . $BASEDIR/LMSybLogRefresh.sh $EnvFileToUse ASE
fi
echo "===========Goodbye FROM PullSybaseErrorLogNEW.sh======================================"
